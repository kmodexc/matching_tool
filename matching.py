    ...
    for i,day in enumerate(DAYS):
        for j,shift in enumerate(SHIFTS):
            ...
            if add_flow is not None:
                ind_people_2 = np.where(add_flow.flow.toarray()[1:(1+df.shape[0]),(1+df.shape[0]+(i*len(SHIFTS))+j)] == 1)
...
def main():
    ...
    day_texts = {"mon": "Monday", "tue": "Tuesday", "wed": "Wednesday", "thr": "Thursday", "fri": "Friday", "sat": "Saturday", "sun": "Sunday"}
    time_texts = {"9": "9:00 - 13:00", "14": "14:00 - 18:00", "18": "18:00 - 22:00"}

    layout = [  [sg.Column([[sg.Text("Excel file:")],[sg.Text("Sheet name:")]]),sg.Column([[sg.InputText(),sg.FileBrowse()],[sg.InputText("ShiftList_KW16")]])],
                [sg.Text("Maximum shift size:"),sg.InputText("3",key="-MAX-SHIFT-SIZE-",size=3)],
                [sg.Button('Generate Shiftplan'),sg.Button('Generate Beerlist'),sg.Text("Comment:"),sg.InputText(key="-COMMENT-",size=25)],
    ...
    while True:
        ...
        if event == "Generate Beerlist":
            ...
            bstr = ""
            for b in beerlist:
                bstr += "|| " + b + """ || Did not fill out shift plan || {X} || """ + date.today().strftime("%d.%m.%Y") + ' || ' + comment + ' ||\n'
